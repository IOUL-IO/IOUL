...
const Page: React.FC = () => {
  useEffect(() => {
    ...
    /* ===== Edge‑click enter‑full‑screen (consistent with other pages) ===== */
useEffect(() => {
  const EDGE_MARGIN = 11;
  function edgeFullscreenHandler({ clientX: x, clientY: y }: MouseEvent) {
  ...
  document.addEventListener("click", edgeFullscreenHandler);
  return () => {
    document.removeEventListener("click", edgeFullscreenHandler);
  };
}, []);
