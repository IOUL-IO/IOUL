...
const html = `<div dangerouslySetInnerHTML={ { __html: `<p style="display:none" lang="en">This page is already in English. No translation is needed.</p>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    // cache base positions in vw units
    itemEls.concat(centerEls).forEach(el => {
    ...
    // apply positions based on current stage
    const applyStage = () => {
      itemEls.forEach(el => {
        const base = parseFloat(el.dataset.baseLeftVw);
        const offset = stage === 0 ? 0 : stage === 1 ? -DIST : -2 * DIST - GAP;
        el.style.transition = 'left ' + 600 + 'ms ease';
        el.style.left = (base + offset) + 'vw';
      });
      centerEls.forEach(el => {
        const base = parseFloat(el.dataset.baseLeftVw);
        const offset = stage < 2 ? 0 : -DIST - GAP;
        el.style.transition = 'left ' + 600 + 'ms ease';
        el.style.left = (base + offset) + 'vw';
      });
    };
    ...
    const rev = document.querySelector('.slide-trigger-reverse');
    const clickFwd = () => {
      setStage(prev => prev === 0 ? 1 : prev === 1 ? 2 : prev);
    };
    const clickRev = () => {
      setStage(prev => prev === 2 ? 1 : prev === 1 ? 0 : prev);
    };
    fwd?.addEventListener('click', clickFwd);
    rev?.addEventListener('click', clickRev);
    return () => {
      fwd?.removeEventListener('click', clickFwd);
      rev?.removeEventListener('click', clickRev);
    };
  }, [stage]);

  return (
    <div className={styles.pageContent} dangerouslySetInnerHTML={{ __html: html }} />
  );
}
