...
function runLegacyScripts() {
  ...
  // Fade out any fixed white mask layers once legacy scripts are run
  Array.from(document.querySelectorAll('.layer-one, .layer-two, .layer-three, .layer-four, .layer-five, .layer-six'))
    .forEach((layer)=>{ 
      (layer as HTMLElement).style.transition='opacity .5s ease-in-out';
