...
useEffect(() => {
  const handleEdgeClick = (event: MouseEvent) => {
    if (itemStage !== 0 || centerStage !== 0) return;  // gate account menu when slides out
...
{
  const widthVW = itemElsRef.current && itemElsRef.current[0]
    ? (itemElsRef.current[0].getBoundingClientRect().width / window.innerWidth) * 100
    : 0;
  move(itemElsRef.current, -(2 * DIST + GAP + widthVW));
}
 // items out first
