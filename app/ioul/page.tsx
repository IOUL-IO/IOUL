...
useEffect(() => {
  ...
  // Slide & edge-click interactions
  const [slideStage, setSlideStage] = useState<'chat'|'heading'|'menu'|'community'|'accountTextsSlid'|'itemFirst'|'center'>('chat');
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      switch (slideStage) {
        case 'chat':
          if (x >= 0 && x <= 6.37) {
            ...
            setSlideStage('accountTextsSlid');
          } else if (x >= 94 && x <= 100) {
