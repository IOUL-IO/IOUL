...
export default function IoulPage() {
  ...
  useEffect(() => {
    const FWD_MIN = 94, FWD_MAX = 100, REV_MIN = 0, REV_MAX = 6;
    const TOP_MIN = 28.5, TOP_MAX = 84, DIST = 60, GAP = 10;
    const DUR = 600, HIDE_BEYOND = 80;
    ...
    const move = (els: HTMLElement[], offsetVw: number) => {
      els.forEach(el => {
    ...
    [...itemEls, ...centerEls].forEach(el => {
      const base = parseFloat(el.dataset.baseLeftVw!);
      if (base < -HIDE_BEYOND || base > 100 + HIDE_BEYOND) {
        el.style.display = 'none';
      }
    });
    let itemStage = 0, centerStage = 0, animating = false;
    const toStage1 = () => { animating = true; itemEls.forEach(el => el.style.display = ''); move(itemEls, -DIST); setTimeout(() => { animating = false; itemStage = 1; }, DUR); };
    const toStage2 = () => { animating = true; move(itemEls, -2 * DIST - GAP); centerEls.forEach(el => el.style.display = ''); move(centerEls, -DIST - GAP); setTimeout(() => { animating = false; itemStage = 2; centerStage = 1; }, DUR); };
    const backToStage1 = () => { animating = true; move(centerEls, 0); setTimeout(() => { centerEls.forEach(el => el.style.display = 'none'); animating = false; centerStage = 0; }, DUR); setTimeout(() => { move(itemEls, -DIST); itemStage = 1; }, DUR); };
    const backToStage0 = () => { animating = true; move(itemEls, 0); setTimeout(() => { itemEls.forEach(el => el.style.display = 'none'); animating = false; itemStage = 0; }, DUR); };
    document.addEventListener('click', e => {
      if (animating) return;
      const xVw = toVw(e.clientX), yVh = toVh(e.clientY);
      if (yVh < TOP_MIN || yVh > TOP_MAX) return;
      if (xVw >= FWD_MIN && xVw <= FWD_MAX) {
        if (itemStage === 0) toStage1();
        else if (itemStage === 1) toStage2();
      } else if (xVw >= REV_MIN && xVw <= REV_MAX) {
